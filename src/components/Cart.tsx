...
/*List of products which Consumer wants to order. */
const Cart = () => {
  const [confirm, setConfirm] = useState<boolean>(false);
  const { dispatch, REDUCER_ACTIONS, cart, totalItems, totalPrice } =
    useContext<useCartContextType>(CartContext);

  /* 
  Function which submit Consumer order. After clicking 
  confirmation button Consumer will be taken to the order summary
  */
  const onSubmit = () => {
  ...
  /* 
  Function which remove all products from cart and clear quantity
  */
  const onNewOrder = () => {
    dispatch({ type: REDUCER_ACTIONS.NEW_ORDER });
    setConfirm(false);
    document.querySelector("html")!.classList.remove("disabled");
  };

  const cartContent = (totalItems: number) => {
    if (totalItems > 0) {
      return (
        /* 
        ...
        */
        <div className="cart">
          ...
          <div className="cart-order-summary">
            <p className="medium-dark-txt semi-bold small-size">Order Total</p>
            <p className="dark-txt bold big-size">{totalPrice}</p>
          </div>
          <div className="carbon-container small-size">
            <svg
          ...
          <button
            className="highlight-button confirm-button full-size normal normal-size"
            onClick={onSubmit}
          >
            Confirm Order
          </button>
  ...
  return (
    <div>
      {confirm ? (
        /*
        Summary message after confirming your order by button. It display
        all products with prices which Consumer ordered.
        */
        <article className="confirmed-container round-border">
          <section>
            <svg
              width="48"
              height="48"
          ...
          <section className="confirmed-order-text">
            <p className="dark-txt bold title-size">Order Confirmed</p>
            <p className="light-txt normal normal-size">
              We hope you enjoy your food!
            </p>
            <div className="confirmed-items-container">
              {cart.map((item) => {
                return <ConfirmCartItem key={item.name} item={item} />;
              })}
              <div className="cart-order-summary">
                <p className="medium-dark-txt semi-bold small-size">
                  Order Total
        ...
        /*
        Cart content when Consumer has an empty cart or has products in cart.
        */
        <div className="cart-container">
          <h2 className="highlight-txt bold big-size">
